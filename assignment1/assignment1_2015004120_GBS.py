...
def greedy_best_first_search(row, col, maze, start_point, goal_points):
    global cnt
    path, priority_queue, point = [], [], None
    visited = [['0' for _ in range(col)] for _ in range(row)]
    priority_queue.insert(0, (start_point, heuristic(start_point, goal_points), (-1, -1)))
        ...
        if point in goal_points:
            ...
            for p in map(lambda d: (point[0] + d[0], point[1] + d[1]), [(1, 0), (-1, 0), (0, 1), (0, -1)]):
                if is_valid(p[0], p[1], row, col, maze, visited):
                    visited[p[0]][p[1]] = '1'
                    priority_queue.insert(0, (p, heuristic(p, goal_points), point))
...
cnt = 0
r, c, m, s, g = get_maze()
a = greedy_best_first_search(r, c, m, s, g)
